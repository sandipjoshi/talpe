...
class FaceDetector {
	...
	public function cropFaceToPng($outFileName = null) {
		if (empty($this -> face)) {
			print "No face detected";
			//throw new NoFaceException('No face detected');
			$canvas = imagecreatetruecolor(125, 125);
			imagecopy($canvas, $this -> canvas, 0, 0, 120, 160, 125, 125);

			if ($outFileName === null) {
				header('Content-type: image/png');
			}

			if ($canvas && imagefilter($canvas, IMG_FILTER_GRAYSCALE)) {
